#!/bin/sh
PREFIX_NAME=aurora-test
SUFFIX_NAME=stack
BASE_STACK_NAME=aurora-test-stack
NETWORK_TEMPLATE_YML=./network_infra.yml
...
while getopts cdu optKey; do
  case "$optKey" in
    c)
      aws cloudformation deploy --template-file $NETWORK_TEMPLATE_YML --stack-name $PREFIX_NAME-network-$SUFFIX_NAME
